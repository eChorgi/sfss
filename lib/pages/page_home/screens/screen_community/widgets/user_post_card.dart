...
class UserPostCard extends StatefulWidget {
  ...
  // static Future<UserPostCard> network({required String imagePath, required String title, required String userName, required String avatarUrl, required int likeCount}) async{
  //     final response = await http.get(Uri.parse(imagePath));
  //     final documentDirectory = await getApplicationDocumentsDirectory();
  //     final file = File('${documentDirectory.path}/temp_image.png');
  //     file.writeAsBytesSync(response.bodyBytes);

  //     // return UserPostCard(imageUrl: localImagePath, title: title, userName: userName, avatarUrl: avatarUrl, likeCount: likeCount);
  //     return UserPostCard(imageUrl: file.path, title: title, userName: userName, avatarUrl: avatarUrl, likeCount: likeCount);
  // }
  @override
...
class _UserPostCardState extends State<UserPostCard> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Navigator.pushNamed(context, '/recordDetail', arguments: {'recordId': 1, 'imageUrl': widget.imageUrl});
      },
